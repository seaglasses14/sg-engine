...
Shader::Shader(const char* vertexPath, const char* fragmentPath, ShaderType pShaderType)
{
	...
	checkCompileErrors(vertex, ShaderErrorCodes::VERTEX);

	fragment = glCreateShader(GL_FRAGMENT_SHADER);
	glShaderSource(fragment, 1, &fShaderCode, NULL);
	glCompileShader(fragment);
	checkCompileErrors(fragment, ShaderErrorCodes::FRAGMENT);

	ID = glCreateProgram();
	glAttachShader(ID, vertex);
	glAttachShader(ID, fragment);
	glLinkProgram(ID);
	checkCompileErrors(ID, ShaderErrorCodes::PROGRAM);
...
void Shader::checkCompileErrors(unsigned int shader, ShaderErrorCodes code)
{
	int success;
	char infoLog[1024];
	if (code == ShaderErrorCodes::PROGRAM)
	{
		glGetProgramiv(shader, GL_LINK_STATUS, &success);
		if (!success)
		{
			glGetProgramInfoLog(shader, 1024, NULL, infoLog);
			LOG_ERROR(std::string("ERROR::PROGRAM_COMPILATION_ERROR\n") + infoLog + "\n-----------------------------\n");
		}
	}
	else
	{
		...
		if (!success)
		{
			...
			if (code == ShaderErrorCodes::VERTEX)
				LOG_ERROR(std::string("ERROR::VERTEX_COMPILATION_ERROR\n") + infoLog + "\n-----------------------------\n");
			else
				LOG_ERROR(std::string("ERROR::FRAGMENT_COMPILATION_ERROR\n") + infoLog + "\n-----------------------------\n");
		}
	}
}